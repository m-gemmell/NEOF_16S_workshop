...
If you would like to view the tree you can open `firefox` and go to __"https://itol.embl.de"__. Then you can upload the `.nwk` to quickly view it.
...
There are various R packages that can be used to analyse 16S data. We have a workshop on how to use the below tools, check our [training page](https://neof.org.uk/training/) to see if it open for registration. Generally these will take the output from QIIME2 so you can carry out further downstream analysis on the QIIME2 created ASV table, taxonomy, and phylogenetic tree.
