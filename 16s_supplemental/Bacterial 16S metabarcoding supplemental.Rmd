...
--observation-metadata-fp final_exports/taxonomy.headers.tsv \
...
while read path ; do \
sample=$(basename $p  |sed "s/_.*//") ; \
echo ${sample},${path},forward ; \
done >> manifest.csv
#Append lines for reverse reads
find /pub39/tea/matthew/NEOF/16sworkshop/RawReads/ -name *R2*fastq.gz | \
while read path ; do \
sample=$(basename $p | sed "s/_.*//") ; \
...
(note to self move this and future chapters up 1 and make chapter 3 about filtering ASVs and samples from a file, possibly also how to merge tables and how to create your own manifest files with ls and find
