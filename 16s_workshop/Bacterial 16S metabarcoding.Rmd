...
<center>
![](figures/intro_logo.png){style="width: 250px; background: white; border-radius: 5px"}
</center>
...
- Import sequencing data and metadata into QIIME2 <u id='artifact_tip'>artifacts</u>.
...
column_1 <- c(
 ...
 '[![](figures/background.png){style="height:150px; border-radius:15px; background:white; border: white solid 5px"}](#background)',
 '[**Cluster and webVNC information**](#cluster)',
 '[![](figures/cluster.png){style="height:150px; border-radius:15px; background:white"}](#cluster)',
 '[**QIIME2 analysis workflow**](#q2workflow)',
 '[![](figures/workflow.png){style="height:150px; border-radius:15px; background:white"}](#q2workflow)',
 '[**PCR primer trimming**](#pcrtrim)',
 '[![](figures/prepare.png){style="height:150px; border-radius:15px; background:white; border: white solid 5px"}](#pcrtrim)',
 '[**Taxonomic classification**](#taxa)',
 '[![](figures/classification.png){style="height:150px; border-radius:15px; background:white"}](#taxa)',
 '[**Rarefaction curve**](#rarefaction)',
 '[![](figures/rarefaction.png){style="height:150px; border-radius:15px; background:white"}](#rarefaction)',
 '[**Differential abundance analysis**](#differential)',
 '[![](figures/balance_scale.png){style="height:150px; border-radius:15px; background:white"}](#differential)',
 '[**Appendix**](#resources)',
 '[![](figures/resources.png){style="height:150px; border-radius:15px; background:white"}](#resources)'
)
column_2 <- c(
  '[**Introduction to QIIME2**](#introqiime2)',
 '[![](figures/intro_logo.png){style="height:150px; border-radius:15px; background:white"}](#introqiime2)',
 '[**Data**](#data)',
 '[![](figures/data_files.png){style="height:150px; border-radius:15px; background:white; border: white solid 5px"}](#data)',
 '[**Sequence import**](#import)',
 '[![](figures/import.png){style="height:150px; border-radius:15px; background:white"}](#import)',
 '[**DADA2 denoising**](#dada2)',
 '[![](figures/clusters.png){style="height:150px; border-radius:15px; background:white"}](#dada2)',
 '[**Phylogenetic tree construction**](#phylogeny)',
 '[![](figures/phylo_tree.png){style="height:150px; border-radius:15px; background:white"}](#phylogeny)',
 '[**Alpha and beta diversity analysis**](#diversity)',
 '[![](figures/diversity.png){style="height:150px; border-radius:15px; background:white"}](#diversity)',
 '[**Final considerations**](#final)',
 '[![](figures/finish.png){style="height:150px; border-radius:15px; background:white; border: white solid 5px"}](#final)',
 "",""
)
...
In the following practical we will use the QIIME2 version released on October 2025 (given the active development on the tool this may not be the most recent release at the time of this workshop). 
...
The QIIME2 pipeline produces and uses <u id='artifact_tip'>artifact</u> files. 
These contain data and metadata and may be of two types: result or visualisation. 

- __Result files (.qza)__: This file type contains the results of a method, which accepts other <u id='artifact_tip'>artifacts</u> and specific settings as input, in order to apply a procedure creating a new <u id='artifact_tip'>artifact</u> (e.g. loading sequences, error correction, <u id='alpha_tip'>alpha-</u> or <u id='beta_tip'>beta</u>-diversity).
...
- Plugins webpage of new QIIME2 site (not all plugins transferred to new site): https://library.qiime2.org/plugins.
...
a. Normalisation by rarefaction. This is performed by reducing all the samples to the same sequence count, by random sub-sampling of the sequences.
...
Methods b-d convert the abundance table into a normalised-abundance table with different strengths and pitfalls, therefore they can affect results for <u id='beta_tip'>beta-diversity</u> and differential abundant analysis.
...
More details of alpha diversity measures can be found in the [appendix](#alphamet).
...
More details of beta diversity measures can be found in the [appendix](#betamet).
...
In this analysis we will use the ANCOM-BC2 method following the current QIIME2 tutorial. 
...
If, in the results, more than 25% of the <u id='asv_tip'>ASVs</u> changes in the examined contrasts, the underlying hypothesis for the ANCOM-BC2 test may be not fulfilled, and there is the chance that the results may be misleading.

Important to note is that ANCOM-BC2 has its own sample-wise normalisation.
Therefore the unnormalised (i.e. pre rarefaction) abundance table shoudd be provided to ANCOM-BC2.
...
<center>
![](figures/cluster.png){style="width:200px; border-radius:15px; background:white; border: white solid 5px"}
</center>
...
<center>
![](figures/nsc200_logon.png){style="width:800px; border-radius:5px; border: #282828 solid 5px"}
</center>
...
<center>
![](figures/terminal_window.png){style="width:700px; border-radius:5px"}
</center>
...
# Data {#data}
<center>
![](figures/data_files.png){style="width:200px; background: white ; border-radius:5px; border: 5px white solid"}
</center>
...
<center>
![](figures/manifest.png){style="width:200px; background: white ; border-radius:5px; border: 5px white solid"}
</center>
...
1. __sample-id__
   ...
   - These must match the sample ids in the [metadata file](#metadatafile).
...
This file will be used in [chapter 7](#import).

## metadata.file.txt {#metadatafile}
<center>
![](figures/tags.png){style="width:200px; background: white ; border-radius:5px; border: 5px white solid"}
</center>

This file contains metadata. It consists of columns separated by tabs. 

It is vital that the "sample-id" column contains the same info as the "sample-id" column in ["PairedEndFastqManifestPhred33.csv"](#manifestfile).
...
For more information on QIIME2 metadata please see: https://use.qiime2.org/en/latest/references/metadata.html
...
This is a fasta file containing the forward and reverse 16S rRNA primer sequences. These are needed for the PCR trimming step ([chapter 8](#pcrtrim)) and the classifier training step ([chapter 10](#taxa)).
...
__Note__: Ensure you include the dot and space (`. `) at the start before `useqiime2-amplicon-2025.10`
...
When you run the QIIME2 commands  in this book you will get the below warning for every/most commands.
The commands are still functioning and we can ignore this warning.

`/pub14/tea/nsc206/miniforge3/envs/qiime2-amplicon-2025.10/lib/python3.10/site-packages/emperor/__init__.py:9: UserWarning: pkg_resources is deprecated as an API. See https://setuptools.pypa.io/en/latest/pkg_resources.html. The pkg_resources package is slated for removal as early as 2025-11-30. Refrain from using this package or pin to Setuptools<81.
  import pkg_resources`

The warning is due to a deprecated, but still functioning, tool in the conda/mamba installation.
This conda environment was created by the QIIME2 developers who will hopefully fix the issue in a future version. 
...
[QIIME2 docs on importing data](https://amplicon-docs.qiime2.org/en/latest/how-to-guides/how-to-import.html)
...
[QIIME2 docs on `qiime demux summarize`](https://amplicon-docs.qiime2.org/en/latest/references/plugins/demux.html#q2-action-demux-summarize)
...
- __`--i-demultiplexed-sequences`__: The input demultiplexed sequence <u id='artifact_tip'>artifact</u>.
...
[QIIME2 docs on `qiime cutadapt trim-paired`](https://amplicon-docs.qiime2.org/en/latest/references/plugins/cutadapt.html#q2-action-cutadapt-trim-paired)
...
If you are working with the ITS region, to trim off the conserved regions from the sequences you can use the ‘q2-itsxpress’ plug in. To install and use this plug-in, please see its specific manual: https://github.com/USDA-ARS-GBRU/itsxpress/tree/main
...
- __`--base-transition-stats`__: Output <u id='artifact_tip'>artifact</u> containing a table listing the transition rates of each ordered pair of nucleotides at each quality score.
- __`--verbose`__: Causes command to display verbose output to screen.

[QIIME2 docs on `qiime dada2 denoise-paired`](https://amplicon-docs.qiime2.org/en/latest/references/plugins/dada2.html#q2-action-dada2-denoise-paired)
...
[QIIME2 docs on `qiime feature-table summarize`]((https://docs.qiime2.org/2024.10/plugins/available/feature-table/summarize/)https://amplicon-docs.qiime2.org/en/latest/references/plugins/feature-table.html#q2-action-feature-table-summarize)
...
[QIIME2 docs on `qiime metadata tabulate`](https://amplicon-docs.qiime2.org/en/latest/references/plugins/metadata.html#q2-action-metadata-tabulate)
...
<center>
![](figures/classification.png){style="width:200px; background: white; border-radius: 5px"}
</center>
...
<center>
![](figures/green_jeans.png){style="width:150px; background: white; border-radius:5px; border: white solid 5px"}
</center>
...
- [Living Tree Project (LTP)](https://imedea.uib-csic.es/mmg/ltp/)
  - Release 08 2023
  -  Greengenes2 uses the full length <u id='s_tip'>16S</u> sequences from LTP
...
<center>
![](figures/greengenes2_phylogeny_tree.png){style="width:600px; border: 5px white solid; border-radius:5px"}
</center>

Greengenes2 can be used on shotgun metagenomic data. This can allow you to effectively compare taxonomic data from <u id='s_tip'>16S</u> and Shotgun metagenomic datasets. 

[Tutorial on classifying shotgun metagenomic data with Greengenes2](https://forum.qiime2.org/t/introducing-greengenes2-2022-10/25291#if-you-have-shotgun-metagenomic-data-6).

For your future analysis you can download the database files from their [ftp site](https://ftp.microbio.me/greengenes_release/2024.09/). However it is also good to check [this site page](http://ftp.microbio.me/greengenes_release/) to see if they have a more recent version when you are analysing your data.
...
<center>
![](figures/scissor_cut.png){style="width:150px; background: white; border-radius: 5px"}
</center>

A classifier training step is required to reach optimum assignment performance.

Prior to training we will extract our <u id='s_tip'>16S</u> region of interest. 
This is carried out by using the amplicon primer sequences. These are the same we used for [removing the PCR primer](#pcrtrim).
...
Another popular <u id='s_tip'>16S rRNA</u> database is [Silva](https://www.arb-silva.de/).

You can download the required files for taxonomic classification from [here](https://library.qiime2.org/data-resources#naive-bayes-classifiers).
`r unhide()`
...
- __`--i-sequences`__: Input <u id='artifact_tip'>artifact</u> containing the feature sequence data.
  - In this case we are using the full length <u id='s_tip'>16S rRNA</u> sequences that are used within the Greenegenes2 phylogenomic tree.
...
- __`--o-reads`__: The output read artifact containing our extracted <u id='s_tip'>16S</u> region as paired reads. This is needed for classifier training.
- __`--p-n-jobs`__: Number of separate processes to use.

[QIIME2 docs on `qiime feature-classifier extract-reads`](https://amplicon-docs.qiime2.org/en/latest/references/plugins/feature-classifier.html#q2-action-feature-classifier-extract-reads)
...
## Classifier training
<center>
![](figures/train.png){style="width:150px; background: white; border-radius: 5px; border: white solid 5px"}
</center>
...
#### Parameters{-}
<center>
![](figures/parameter_green.png){style="width:100px; background: white; border-radius: 5px; border: 5px solid white"}
</center>

- __`--i-reference-reads`__: Input <u id='artifact_tip'>artifact</u> containing the extracted <u id='s_tip'>16S</u> region as paired reads.
- __`--i-reference-taxonomy`__: Input <u id='artifact_tip'>artifact</u> containing the taxonomy information of the features.
  - This was downloaded from: https://ftp.microbio.me/greengenes_release/2024.09/2024.09.backbone.tax.qza.
- __`--o-classifier`__: Output <u id='artifact_tip'>artifact</u> containing the trained classifier. This will be used for feature (<u id='asv_tip'>ASV</u>) taxonomy classification.

[QIIME2 docs on `qiime feature-classifier fit-classifier-naive-bayes`](https://amplicon-docs.qiime2.org/en/latest/references/plugins/feature-classifier.html#q2-action-feature-classifier-fit-classifier-naive-bayes)
...
With our trained classifier we can assign taxonomic assignments to the <u id='asv_tip'>ASV</u> representative sequences.
...
#### Parameters{-}
<center>
![](figures/parameter_orange.png){style="width:100px; background: white; border-radius: 5px; border: 5px solid white"}
</center>

- __`--i-classifier`__: Input <u id='artifact_tip'>artifact</u> containing the trained classifier.
- __`--p-confidence`__: Confidence threshold for limiting taxonomic depth.
  ...
  - This could cause an <u id='asv_tip'>ASV</u> to be classified to Genus level but not species level as the species level assignment had a confidence of <0.7 whilst the confidence of the genus and higher assignments was >=0.7.
- __`--i-reads`__: Input <u id='artifact_tip'>artifact</u> containing the representative sequences of the features (<u id='asv_tip'>ASVs</u>) to be classified.
- __`--o-classification`__: Output <u id='artifact_tip'>artifact</u> containing the taxonomic assignments of the features (<u id='asv_tip'>ASVs</u>).
- __`--p-n-jobs`__: Number of separate processes to use.

[QIIME2 docs on `qiime feature-classifier classify-sklearn`](https://amplicon-docs.qiime2.org/en/latest/references/plugins/feature-classifier.html#q2-action-feature-classifier-classify-sklearn)

## Taxonomy bar chart production {#taxabar}
<center>
![](figures/bar_charts.png){style="width:150px; background: white; border-radius: 5px; border: 5px white solid"}
</center>

Create a taxonomy-abundance bar-chart visualisation <u id='artifact_tip'>artifact</u>:
...
- __`--i-table`__: Input <u id='artifact_tip'>artifact</u> containing the <u id='asv_tip'>ASV</u> abundance table.
- __`--i-taxonomy`__: Input <u id='artifact_tip'>artifact</u> containing the taxonomic classifications of the <u id='asv_tip'>ASVs</u>.
...
[QIIME2 docs on `qiime taxa barplot`](https://amplicon-docs.qiime2.org/en/latest/references/plugins/taxa.html#q2-action-taxa-barplot)

## Taxonomy: MCQs
<center>
![](figures/question_bubble_blue.png){style="width:150px; background: white; border-radius: 5px; border: white solid 5px"}
</center>
...
Excellent! We have trimmed and denoised our data, producing <u id='asv_tip'>ASVs</u>. This has been followed by assigning taxonomic classifications to the <u id='asv_tip'>ASVs</u>. 
...
You may want to change some steps and/or settings of what you have done so far if the composition does not match what you expect. For example, change the de-noising tool or apply more quality filters to further improve the quality of the sequence under examination.
...
<center>
![](figures/rarefaction.png){style="width:200px; border-radius: 5px ; background: white; border: 5px solid white"}
</center>

The detected diversity partly depends on the sampling depths. 
An incompletely sampled community will appear less diverse than a fully sampled community. 

To compare the diversity between samples we need to account for their different sampling depths. 
This can be carried out with rarefaction. 
...
Equally sized subsamples are directly comparable among different samples. 
Additionally, we can progressively subsample from a sample to draw a curve of estimated <u id='alpha_tip'>alpha</u> diversity. 
The shape of this curve is informative. 
If the curve plateaus, then increased sampling depth will not increase the estimate of <u id='alpha_tip'>alpha</u> diversity (the community is fully sampled). 
...
```
<center>
![](figures/rarefaction_ggplot.png){style="width:800px; border-radius:5px"}
</center>
...
## Rarefaction: run
<center>
![](figures/play_blue.png){style="width:150px; border-radius: 5px ; background: white; border: 5px solid white"}
</center>

Given an abundance table (and a phylogenetic tree for calculating phylogenetic distance), we can plot rarefaction curves using different <u id='alpha_tip'>alpha</u>-diversity metrics. 
Here, we will use diversity metrics ([observed features](#obvs), [Simpson](#simpson), [Shannon](#shannon), [Faith’s PD](#faith)) as well as an evenness metric ([Simpson’s evenness](#simpsone)).
...
- __`--m-metadata-file`__: [Metadata file](#metadata.file.txt) for our samples.
- __`--p-metrics`__: This parameter indicates which <u id='alpha_tip'>alpha</u> diversity to use for rarefaction.
  - This parameter can be specified multiple times for different metrics.
- __`--o-visualization`__: Output visualisation <u id='artifact_tip'>artifact</u> containing the rarefaction plots. 

[QIIME2 docs on `qiime diversity alpha-rarefaction`](https://amplicon-docs.qiime2.org/en/latest/references/plugins/diversity.html#q2-action-diversity-alpha-rarefaction)

### Copy rarefaction output {#cprarefaction}
<center>
![](figures/copy.png){style="width:150px; border-radius: 5px ; background: white; border: 5px solid white"}
</center>
...
<center>
![](figures/eye_blue.png){style="width:200px; border-radius: 5px ; background: white; border: 5px solid white"}
</center>

Load the output visualisation <u id='artifact_tip'>artifact</u> into QIIME2 view. 
You can select the <u id='alpha_tip'>alpha</u> diversity __Metric__ to visualise as well as the groupings (__Sample Metadata Column__, where each __BarcodeSequence__ represents a sample). Generally the [__observed_features__](#obvs) metric is the best __Metric__ to view.

The visualisation has two plots. 

- __Top plot: <u id='alpha_tip'>Alpha</u> rarefaction plot.__
...
### Rarefaction: MCQs
<center>
![](figures/question_bubble_blue.png){style="width:150px; border-radius: 5px ; background: white; border: 5px solid white"}
</center>
...
```
1. At approximately what sequencing depth do the rarefaction curves of most of the samples (>90%) plateau?  `r longmcq(opts_p)`
...
## Rarefaction: summary
<center>
![](figures/sum_blue.png){style="width:150px; border-radius: 5px ; background: white; border: 5px solid white"}
</center>
...
Load the produced files (`shannon-group-significance.kw.qzv` & `shannon-group-significance.rs.qzv`) into QIIME2 view using 2 firefox tabs. At this point I will remind you to have only one firefox window open which is to be left open. Also make sure to close tabs you don't need any more (always leaving one open).
...
<center>
![](figures/beta_div_2.png){style="width:300px; border-radius: 5px ; background: white; border: 5px solid white"}
</center>

We'll now carry out statistical analysis of the <u id='beta_tip'>beta</u> diversity ordination plots. 
We'll use __'PERMANOVA'__ analysis to determine if the separation of samples by metadata groups has statistical significance or if it may have just originated by chance.
...
## Beta diversity: run 
<center>
![](figures/play_green.png){style="width:150px; border-radius: 5px ; background: white; border: 5px solid white"}
</center>
...
- __`--i-distance-matrix`__: Input <u id='artifact_tip'>artifact</u> files containing the <u id='beta_tip'>beta</u> diversity values.
- __`--m-metadata-file`__: [Metadata file](#metadata.file.txt) for our samples.
...
- __`--p-permutations`__: The number of permutations to run when computing p-values.
  - The default is `999`.
- __`--o-visualization`__: The output <u id='artifact_tip'>artifact</u> containing the visualisation.

[QIIME2 docs on `qiime diversity beta-group-significance`](https://amplicon-docs.qiime2.org/en/latest/references/plugins/diversity.html#q2-action-diversity-beta-group-significance)
...
### Beta diversity: MCQs
<center>
![](figures/question_bubble_green.png){style="width:150px; border-radius: 5px ; background: white; border: 5px solid white"}
</center>
...
<center>
![](figures/sum_green.png){style="width:150px; border-radius: 5px ; background: white; border: 5px solid white"}
</center>

Viewing the <u id='beta_tip'>beta</u> diversity ordination plots and the statistical output we can see there is a difference between the Corridor and Toilet samples. However, there does not appear to be a difference between the Entrance and MainBuilding samples from our quick look.
...
If you want to carry out multi-way ADONIS tests you can use the [`qiime diversity adonis` plugin](https://amplicon-docs.qiime2.org/en/latest/references/plugins/diversity.html#q2-action-diversity-adonis).
...
Are the results you obtained with this pipeline making any sense? 
In a real scenario, you (as the owner of the dataset) are the one with the deepest knowledge of the dataset you are analysing, so it is upon you to answer this question. 
This answer needs to be inferred by gathering all the information you can on the samples, from literature or biochemical. 
If the answer is ‘no’, you can consider the following points.
...
# Resources
<center>
![](figures/resources.png){style="width:200px; border-radius: 5px ; background: white; border: 5px solid white"}
</center>

## Suggested readings

The QIIME2 tutorials page contains many useful protocols and possible usage for these scripts (https://docs.qiime2.org/2024.10/tutorials/).
...
To rarefy or not to rarefy: robustness and efficiency trade-offs of rarefying microbiome data, Johnny Hong, Ulas Karaoz, Perry de Valpine, William Fithian (2022) Oxford academic Bioinformatics

https://academic.oup.com/bioinformatics/article/38/9/2389/6536959
...
To create the mamba environment `qiime2-amplicon-2025.10` run the below commands in your bash. You will need to have installed `mamba` first.
...
__Note__: You may want to install the most recent version of QIIME2. Please see the following page for installation instructions: https://library.qiime2.org/quickstart/amplicon.

# Diversity metrics
<center>
![](figures/metric_square.png){style="width:200px; border-radius: 5px ; background: white; border: 5px solid white"}
</center>
...
<center>
![](figures/faith_pd.png){style="width:600px; border-radius:5px"}
</center>
...
## Beta diversity metrics {#betamet}
<center>
![](figures/beta_div_2.png){style="width:200px; border-radius: 5px ; background: white; border: 5px solid white"}
</center>
